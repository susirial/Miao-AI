...
const outputDir = resolve(root, 'scripts/fixtures/agnes-30-flash')
...
const matches25 = explicitFalseWorks
  && thinkingSeparates
  && forcedToolWorks
  && thinking.reasoningFields.includes('reasoning_content')

const report = `# Agnes 3.0 Flash Protocol Probe
...
- Adapter reuse: ${matches25 ? '3.0 matches the frozen 2.5 Flash strategy; keep the shared Agnes Chat Completions adapter unchanged' : '3.0 diverged from 2.5; adapter changes must be catalog-id scoped so 2.5 does not regress'}.
...
database.close()
console.log('Agnes 3.0 Flash protocol probe completed. Review scripts/fixtures/agnes-30-flash/probe-report.md.')
